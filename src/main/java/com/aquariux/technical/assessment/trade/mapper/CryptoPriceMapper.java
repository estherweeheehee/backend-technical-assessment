...
public interface CryptoPriceMapper {
    ...
    @Select("""
            ...
            FROM crypto_prices cp 
            INNER JOIN crypto_pairs pair ON cp.crypto_pair_id = pair.id 
            WHERE cp.created_at = (SELECT MAX(created_at) FROM crypto_prices cp2 WHERE cp2.crypto_pair_id = cp.crypto_pair_id) AND cp.crypto_pair_id = #{givenPairId}
            ORDER BY cp.crypto_pair_id
            """)
    Optional<CryptoPrice> findLatestPriceByPairId(Long givenPairId);
}